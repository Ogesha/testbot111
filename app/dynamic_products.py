...
async def replace_all_categories_and_products(session: AsyncSession, categorized: dict[str, list[dict]]):
    ...
    # выбрасывается InvalidRequestError. Это приводило к тому, что после
    # первого успешного обновления последующие падали с ошибкой и таблицы
    # не пересоздавались. Явно очищаем метаданные перед повторным
    # построением таблиц.
    _meta.clear()
